...
class LogIngestionService:
    ...
    def ingest_to_db(
        ...
        *,
        ...
        """
        Full pipeline: format parsing → semantic processing → SQLite.

        The format parsing layer handles format detection, parsing, encoding
        normalisation, and corruption detection.  The semantic pipeline handles
        semiconductor field mapping, unit conversion, LLM-assisted mapping for
        unknown vendor fields, anomaly detection, and database storage.

        Returns a BatchResult (total, success, failed, cache_hits, llm_calls).
        """
        import sys
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        from ingestion.batch import Pipeline as P2Pipeline, _run_pipeline, BatchResult
        from database.writer import insert_failure

        if pipeline is None:
            pipeline = P2Pipeline()

        log_source = source_label or str(path)

        # Fresh staging so previous ingest() calls don't bleed in
        temp_staging = StagingArea(backend="memory", deduplicate=True)
        temp_service = LogIngestionService(staging=temp_staging)
        temp_service.ingest_file(path, source_label=log_source)

        result = BatchResult()

        # Corrupted records go straight to parse_failures (no LLM spend)
        for rec in temp_staging.all_records(include_corrupted=True):
            if rec.corrupted:
                insert_failure(
        ...
        raw_records = []
        for rec in temp_staging.all_records(include_corrupted=False):
        ...
        if raw_records:
            _run_pipeline(raw_records, "unknown", pipeline, result, log_source)
    ...
    def _get_processor(self, fmt: str):
        ...
        cls = self._PROCESSOR_MAP.get(fmt, LogTemplateTrie)
